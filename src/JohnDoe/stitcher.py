...
class PanaromaStitcher():
    ...
    def resize_image(self, image):
        return self.resize_image_custom(image, self.scale_percent)
    ...
    def make_panaroma_for_images_in(self, path, lowe_ratio=0.75, max_threshold=4.0):
        ...
        for i in range(1, len(images)):
            ...
            result_image = self.warp_images_custom(result_image, homography, (result_image.shape[0] + current_image.shape[0], result_image.shape[1] + current_image.shape[1]))
    ...
    def match_keypoints(self, keypoints_A, keypoints_B, features_A, features_B, lowe_ratio, max_threshold):
        ...
        if len(good_matches) > 4:
            ...
            homography = self.find_homography_custom(points_A, points_B, threshold=max_threshold)
            return good_matches, homography
        return None, None

# Helper functions for custom operations

    def resize_image_custom(self,image, scale_percent):
    ...
    def perspective_transform_custom(self,points, homography):
        transformed_points = []
        for point in points:
            x, y = point[0]
    ...
    def find_homography_custom(self,points_A, points_B, threshold=3.0, iterations=2000):
        max_inliers = 0
    ...
    def warp_images_custom(self,image, homography, output_shape):
